Shader "Unlit/PreprocessedDepthToTransparency"
{
    Properties
    {
        _Near ("Near", Range(0, 1)) = 0.2
        _Far ("Far", Range(0, 1)) = 0.8
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                float d = UNITY_SAMPLE_TEX2DARRAY(_PreprocessedEnvironmentDepthTexture, float3(duv, eye)).r;

                float nearVal = min(_Near, _Far);
                float farVal = max(_Near, _Far);
                bool inRange = (d >= nearVal) && (d <= farVal);
